...
function buildFormFromVehicle(
  vehicle: Vehicle,
  loadedBrands: BrandRecord[],
): VehicleFormState {
  const matchedBrand = loadedBrands.find((brand) => brand.name === vehicle.brand);
  ...
  return {
    ...
    iva: vehicle.iva,
    imageGalleryText: getVehicleImages(vehicle).join("\n"),
  };
}
...
export default function EditVehiclePage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        const [loadedBrands, loadedModels, loadedVehicle] = await Promise.all([
          fetchBrandsFromFirestore(),
          fetchModelsFromFirestore(),
          fetchVehicleByIdFromFirestore(params.id),
        ]);
        ...
        if (loadedVehicle) {
          setForm(buildFormFromVehicle(loadedVehicle, loadedBrands));
        }
      } catch {
        if (cancelled) return;
        setBrands([]);
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-white/70">
              Modifica los datos del anuncio y guarda los cambios en la
              colección `vehicles` de Firestore.
            </p>
          </div>
        ...
        >
          ...
          {!brands.length ? (
            <div className="mt-6 rounded-lg border border-amber-400/20 bg-amber-500/10 p-4 text-sm text-amber-100">
              Crea al menos una marca y un modelo en el panel admin antes de
              editar un coche.
            </div>
          ) : null}
